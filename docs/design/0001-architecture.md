...
| Deferral (open, not built) | Server-side policy on the reboot block, two fields: how many times a logged-in user may postpone, and for how long each time. Absent or zero means today's behavior, so an old server changes nothing. The count is kept per reason and reset when the reason is satisfied; the last countdown cannot be dismissed. Deferral is a property of the reason, not the coordinator: a software install may say "postpone up to 3 times", an imaging task keeps the hard grace countdown. Headless Linux has nobody to ask and falls through to the existing rule |
...
| Directory placement | OU set once, at join; a later OU change on the server never moves the machine | A computer object lives in exactly one OU (it is the DN's container), so `ou` is a single string and ensure means move on mismatch, an LDAP rename with the join credential. "Several OUs" is really security groups: `groups` is a list, ensured by diffing membership. Converged is read back from the object's DN, not from the join call's return code | With the `ad` type |
...
| User tracking: refactor, not a port | The legacy client polls the session list and posts a bare login or logout event with a username and a timestamp. It is a best guess: a missed poll loses the event, and a login with no matching logout leaves the server unable to say whether the user is still there or the machine lost power | Take the events from the OS instead of inferring them: logind over D-Bus on Linux, WTS session-change notifications (or Security log 4624/4634) on Windows, the SystemConfiguration console-user notification on macOS. Each event carries a session id, the machine's boot id, the kind of session (console, RDP, SSH) and a reason where the OS gives one (user logout, forced logout by the agent, shutdown). A login whose previous session never closed is then explainable: same boot id means an interrupted session, a new boot id means the machine went down uncleanly, and the agent can also post a synthetic close for it on startup using the last known state. The server representation changes to match: sessions with a start, an end and an end reason, rather than two disconnected event rows. Auto logout becomes one of those end reasons | v1, replaces the legacy shape |
